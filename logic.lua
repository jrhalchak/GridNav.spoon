...
function M.cutRight(spoon)
  ...
  local newFrame = {
    x = spoon.state.currentGridFrame.x + spoon.state.currentGridFrame.w * cutAmount,
    y = spoon.state.currentGridFrame.y,
    w = utils.cutPercent(spoon.state.currentGridFrame.w, cutAmount),
    h = spoon.state.currentGridFrame.h
  }
...
function M.cutUp(spoon)
  ...
  local newFrame = {
    ...
    h = utils.cutPercent(spoon.state.currentGridFrame.h, cutAmount)
  }
  spoon.state.isFullScreenGrid = false
  M.updateGridFrame(spoon, newFrame)
end
...
function M.cutDown(spoon)
  ...
  local newFrame = {
    x = spoon.state.currentGridFrame.x,
    y = spoon.state.currentGridFrame.y + spoon.state.currentGridFrame.h * cutAmount,
    w = spoon.state.currentGridFrame.w,
    h = utils.cutPercent(spoon.state.currentGridFrame.h, cutAmount)
  }
  spoon.state.isFullScreenGrid = false
...
function M.moveLeft(spoon)
  local distance = utils.validateCutMovePercentage(spoon.config.moveDistance, "move")
  if spoon.state.isFullScreenGrid then return end
  local newFrame = {
    x = spoon.state.currentGridFrame.x - (spoon.state.currentGridFrame.w * distance),
    y = spoon.state.currentGridFrame.y,
    w = spoon.state.currentGridFrame.w,
    h = spoon.state.currentGridFrame.h
  }
...
function M.moveUp(spoon)
  ...
  local newFrame = {
    x = spoon.state.currentGridFrame.x,
    y = spoon.state.currentGridFrame.y - (spoon.state.currentGridFrame.h * distance),
    w = spoon.state.currentGridFrame.w,
    h = spoon.state.currentGridFrame.h
  }
  M.updateGridFrame(spoon, newFrame)
...
local click = function(clickPoint, clickState, id)
  hs.eventtap.event.newMouseEvent(hs.eventtap.event.types["leftMouseDown"], clickPoint):setProperty(clickState, id):post()
  hs.eventtap.event.newMouseEvent(hs.eventtap.event.types["leftMouseUp"], clickPoint):setProperty(clickState, id):post()
end

--- Double-click on the target in the center of the grid's current location
-- @param spoon The GridNav spoon instance
-- @return nil
function M.doubleClick(spoon)
  local cf = spoon.state.currentGridFrame
  local targetX = cf.x + cf.w / 2
  local targetY = cf.y + cf.h / 2

  -- Perform double-click by creating two click events with proper timing
  local clickPoint = { x = targetX, y = targetY }
  local clickState = hs.eventtap.event.properties.mouseEventClickState

  click(clickPoint, clickState, 1)
  hs.timer.usleep(1000)
  click(clickPoint, clickState, 2)

  M.exitGridNavMode(spoon)
end

--- Triple-click on the target in the center of the grid's current location for selection (because click+drag is inconsistent)
-- @param spoon The GridNav spoon instance
-- @return nil
function M.tripleClick(spoon)
  local cf = spoon.state.currentGridFrame
  local targetX = cf.x + cf.w / 2
  local targetY = cf.y + cf.h / 2
